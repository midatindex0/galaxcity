...
class Info(commands.Cog):
    ...
    @commands.command(
        name="locate",
        aliases=["iss", "location", "loc"],
        help="Get the location of the ISS",
    )
    ...
    @commands.command(
        name="pic-of-day",
        aliases=["potd", "pod", "picture-of-the-day", "picofday"],
        help="Get the picture of the day for NASA",
    )
    async def potd(self, ctx):
        """GET THE PICTURE OF THE DAY FOR NASA"""
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://api.nasa.gov/planetary/apod?api_key=1VPdcmokG1qNJ7wv3VC2H8SQS48OXfytFBtEnRkC"
            ) as response:
                json = await response.json()
            em = (
                discord.Embed(
                ...
                .add_field(name="Date", value=json["date"], inline=False)
                .add_field(
                    name="URL", value=f"[click here]({json['url']})", inline=False
                )
