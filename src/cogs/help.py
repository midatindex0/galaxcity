...
class Help(commands.Cog):
    ...
    @commands.group(
        name="help",
        aliases=["h", "halp", "commands", "cmds", "command", "cmd"],
    ...
    async def help(self, ctx):
        ...
        em.add_field(
            name="Story", value="`play` **⌬** `reset` **⌬** `profile`", inline=False
        )
        em.add_field(name="Space", value="`locate` **⌬** `pic-of-day`", inline=False)
        em.add_field(
            name="Miscellaneous",
            value="`credits` **⌬** `story` **⌬** `ping` **⌬** `uptime`",
            inline=False,
        )
    ...
    async def reset(self, ctx):
        ...
        em.add_field(name="Aliases", value="`g!r`", inline=False)
    ...
    async def play(self, ctx):
        em = discord.Embed(
            title="g!play", description="Play the game", colour=discord.Colour.purple()
        )
        em.set_thumbnail(url=self.bot.user.display_avatar)
        em.set_footer(
            text=f"Requested by {ctx.author.name}", icon_url=ctx.author.display_avatar
        )
        em.add_field(name="Aliases", value="`g!p`", inline=False)
        await ctx.send(embed=em)

    @help.command(name="profile", aliases=["prof", "id", "pf"])
    async def profile(self, ctx):
        ...
        em.add_field(
            name="Aliases", value="`g!pf` **⌬** `g!id` **⌬** `g!prof`", inline=False
        )
    ...
    @help.command(name="credits", aliases=["creds", "credit", "cred"])
    ...
    @help.command(name="locate", aliases=["iss", "location", "loc"])
    ...
    @help.command(
        name="pic-of-day", aliases=["potd", "pod", "picture-of-the-day", "picofday"]
    )
